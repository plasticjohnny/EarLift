/**
 * Cymatic Harmonic Visualization - WebGL Implementation
 * High-performance GPU-accelerated interference pattern renderer
 */

class CymaticHarmonicVizWebGL {
    constructor(canvasId = 'cymaticHarmonicCanvasWebGL') {
        ...
        // WebGL-specific settings (separate from Canvas 2D version)
        this.settings = {
            // Core visualization
            amplitude: 1.0,
            harmonicOrder: 5,
            colorMode: 'piano', // 'piano' or 'continuous'
            ...
            // Intensity settings (higher defaults for visibility)
            rootIntensity: 1.0,
            intervalIntensity: 0.8,
            intersectionIntensity: 1.2,

            // Beat effects
            showBeatBrightness: false,
            beatBrightnessIntensity: 0.0,
            ...
            ringBreathing: false,
            ringBreathingIntensity: 0.5,
            heatMap: false,
            heatMapIntensity: 0.5,
            nodeMigration: false,
            nodeMigrationSpeed: 1.0,
            nodeMigrationAnimate: false,

            // Advanced node migration modes
            nodeDensityFlow: false,
            nodeDensityIntensity: 1.0,
            phaseTrails: false,
            phaseTrailLength: 1.0,
            nodeCoalescence: false,
            coalescenceStrength: 1.0,
            flowField: false,
            flowTurbulence: 1.0,
            depthLayers: false,
            depthLayerCount: 4,
        ...
        // Piano chromatic colors (HSL converted to RGB)
        this.chromaticColors = [
            ...
            [0.0, 1.0, 0.0],   // E - Green
            [0.0, 1.0, 0.5],   // F - Cyan-Green
            [0.0, 1.0, 1.0],   // F# - Cyan
            [0.0, 0.5, 1.0],   // G - Blue-Cyan
            [0.0, 0.0, 1.0],   // G# - Blue
            [0.5, 0.0, 1.0],   // A - Purple
            [1.0, 0.0, 1.0],   // A# - Magenta
            [1.0, 0.0, 0.5]    // B - Red-Magenta
        ];
        ...
        console.log('WebGL Cymatic Visualization initialized');
    }
    ...
    getFragmentShaderSource() {
        return `
            ...
            void main() {
                ...
                vec3 tone1Contribution = uTone1Color * (0.25 + brightness1 * 0.35) * falloff1;
                vec3 tone2Contribution = uTone2Color * (0.25 + brightness2 * 0.35) * falloff2;
                ...
                if (overlapping) {
                    ...
                    vec3 baseColor = (uTone1Color + uTone2Color) * 0.3;
    ...
    updateUniforms() {
        ...
        if (window.cymaticHarmonicViz) {
            this.isPlayingTone1 = window.cymaticHarmonicViz.isPlayingTone1;
            this.isPlayingTone2 = window.cymaticHarmonicViz.isPlayingTone2;
            this.tone1Freq = window.cymaticHarmonicViz.tone1Freq;
            this.tone2Freq = window.cymaticHarmonicViz.tone2Freq;
            this.tone1X = window.cymaticHarmonicViz.tone1X / 100; // Convert from percentage
            this.tone1Y = 0.5; // Center vertically
            this.tone2X = window.cymaticHarmonicViz.tone2X / 100;
            this.tone2Y = 0.5;
            this.rootTone = window.cymaticHarmonicViz.rootTone;

            // Sync reveal radii for expanding wave effect
            const tone1RevealRadius = window.cymaticHarmonicViz.getRevealRadius(
                window.cymaticHarmonicViz.tone1StartTime,
                window.cymaticHarmonicViz.tone1StopTime,
                this.isPlayingTone1,
                window.cymaticHarmonicViz.tone1X,
                window.cymaticHarmonicViz.tone1Y,
                width,
                height
            );
            const tone2RevealRadius = window.cymaticHarmonicViz.getRevealRadius(
                window.cymaticHarmonicViz.tone2StartTime,
                window.cymaticHarmonicViz.tone2StopTime,
                this.isPlayingTone2,
                window.cymaticHarmonicViz.tone2X,
                window.cymaticHarmonicViz.tone2Y,
                width,
                height
            );
            ...
            this.beatAmplitude = window.cymaticHarmonicViz.currentBeatAmplitude || 0;

            // Sync settings
            this.settings.tone1Color = window.cymaticHarmonicViz.settings.tone1Color;
            this.settings.tone2Color = window.cymaticHarmonicViz.settings.tone2Color;
            this.settings.colorMode = window.cymaticHarmonicViz.settings.colorMode;
            this.settings.rootFreq = window.cymaticHarmonicViz.settings.rootFreq;
            this.settings.amplitude = window.cymaticHarmonicViz.settings.amplitude;
            this.settings.harmonicOrder = window.cymaticHarmonicViz.settings.harmonicOrder;
            this.settings.useBoundary = window.cymaticHarmonicViz.settings.useBoundary;
            this.settings.boundaryRadius = window.cymaticHarmonicViz.settings.boundaryRadius;
            this.settings.speakerRadius = window.cymaticHarmonicViz.settings.speakerRadius;
            this.settings.useFoggyEdge = window.cymaticHarmonicViz.settings.useFoggyEdge;
            this.settings.foggyEdgeStart = window.cymaticHarmonicViz.settings.foggyEdgeStart;
            this.settings.rootIntensity = window.cymaticHarmonicViz.settings.rootIntensity;
            this.settings.intervalIntensity = window.cymaticHarmonicViz.settings.intervalIntensity;
            this.settings.intersectionIntensity = window.cymaticHarmonicViz.settings.intersectionIntensity;
        }
    ...
    render() {
        ...
        if ((this.isPlayingTone1 || this.isPlayingTone2) && !this._debugLogged) {
            console.log('WebGL Debug (tones playing):', {
                ...
                syncedFrom: window.cymaticHarmonicViz ? 'Canvas2D' : 'none'
            });
